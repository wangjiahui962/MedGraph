...
- **全量图**：加载全部节点和关系，以快速同心布局展示总体规模。大图主要用于鸟瞰，不适合逐条阅读。

节点采用语义缩放：放大画布时，节点在屏幕上的增长速度低于画布缩放速度，文字和连线不会随画布无限变粗变大；当前可见区域内的重要名称会经过碰撞避让后逐步显示。实体类型同时使用颜色与形状编码。图例位于画布外的可折叠区域，不遮挡图谱。
